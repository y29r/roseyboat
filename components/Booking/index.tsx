...
export default function Booking() {
	...
	function decrementNights(): void {
		if (nights > MIN_NIGHTS) {
			setSpinDirection("down");
			setNights(nights => nights - 1);
		}
	}

	function incrementNights(): void {
						...
						<div className="bg-white rounded-2xl shadow-sm border border-beige/40 p-6 lg:p-8">
							...
							<div className="mb-6 pb-6 border-b border-beige/40">
								<div className="flex items-baseline gap-2 mb-4">
									<span className="font-serif text-4xl text-dark font-light">€{RATE_PER_NIGHT}</span>
								...
								<div className="flex flex-col items-center gap-3 sm:flex-row sm:items-center sm:justify-between bg-cream rounded-xl px-4 py-3">
									<div className="flex items-center gap-3">
										<button
											...
											className={`w-8 h-8 rounded-full border border-beige flex items-center justify-center font-sans text-lg transition-colors ${nights <= MIN_NIGHTS ? "opacity-30 pointer-events-none text-muted" : "text-muted hover:border-canal-green hover:text-canal-green"}`}
										>−</button>

										<div className="overflow-hidden h-6 w-24 flex items-center justify-center">
											<span
												key={nights}
										...
										<button
											...
											className={`w-8 h-8 rounded-full border border-beige flex items-center justify-center font-sans text-lg transition-colors ${nights >= MAX_NIGHTS ? "opacity-30 pointer-events-none text-muted" : "text-muted hover:border-canal-green hover:text-canal-green"}`}
										>+</button>
									</div>
									<div className="text-center sm:text-right">
										...
										>
											<span className="inline-flex items-center gap-1 font-sans text-xs bg-canal-green/10 text-canal-green px-2 py-0.5 rounded-full">
