...
const MOMENT_IMAGES: { image: string; alt: string }[] = [
...
export default function Experience() {
	...
	const moments: Moment[] = translations.experience.moments.map(
		(moment: { tag: string; heading: string; text: string }, index: number) => ({ ...moment, ...MOMENT_IMAGES[index] })
	);
			...
			<div className="max-w-7xl mx-auto px-6 lg:px-10">
				...
				<div className="flex flex-col gap-20 lg:gap-28">
					{moments.map((moment: Moment, index: number) => (
						<div
							key={moment.tag}
							className={`grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center ${index % 2 === 1 ? "lg:flex-row-reverse" : ""
								}`}
						>
							<ParallaxImage
								src={moment.image}
								...
								className={`aspect-[4/3] rounded-2xl shadow-md w-full${index % 2 === 1 ? " lg:order-2 reveal-right" : " reveal-left"}`}
							/>
							<div className={index % 2 === 1 ? "lg:order-1 reveal-left" : "reveal-right"}>
