...
export default function Navbar() {
	...
	useEffect(() => {
		const ids: string[] = NAV_HREFS.map((navItem) => navItem.href.slice(1));
	...
	const navLinks: { label: string; href: string }[] = NAV_HREFS.map((navItem) => ({
		label: translations.nav[navItem.key as keyof typeof translations.nav] as string,
		href: navItem.href,
	}));
			...
			<div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between h-16 lg:h-20">
				...
				<nav className="hidden md:flex items-center gap-8">
					{navLinks.map((link: { label: string; href: string }) => {
						const isActive: boolean = activeSection === link.href.slice(1);
						...
						{LANGUAGES.map((mappedLanguage: Language, index: number) => (
							<Fragment key={mappedLanguage}>
								...
								<button
									...
									className={`font-sans text-xs tracking-widest transition-colors duration-300 ${mappedLanguage === language
			...
			>
				<nav className="flex flex-col px-6 py-4 gap-4">
					...
					<div className="flex items-center gap-3 pt-3 mt-1 border-t border-beige/30">
						<span className="font-sans text-xs text-muted">{translations.nav.language}</span>
						<div className="flex items-center gap-1.5">
							{LANGUAGES.map((mappedLanguage: Language, index: number) => (
								<Fragment key={mappedLanguage}>
									...
									<button
										onClick={() => setLanguage(mappedLanguage)}
										className={`font-sans text-xs tracking-widest transition-colors ${mappedLanguage === language ? "text-canal-green font-semibold" : "text-muted hover:text-dark"
											}`}
									>
										{mappedLanguage}
									</button>
